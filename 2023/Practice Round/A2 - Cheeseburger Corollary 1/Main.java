...
/*
    Solution Created: 16:38:29 07/10/2023
    Custom Competitive programming helper.
*/
public class Main {
  static boolean fileRead = true;

  static long sCost, dCost, c;

  public static long get(long sTake) {
    long cLeft = c - sCost * sTake;
    if (cLeft < 0) {
      Util.dbg("error!");
      return 0;
    }
    long dTake = cLeft / dCost;
    if (sTake == 0)
      return dTake * 2 - 1;
    return dTake * 2 + sTake;
  }
  ...
  public static void solve() {
    ...
    dCost = in.nextLong();
    c = in.nextLong();
    long ans = 0, maxStake = c / sCost, X = (long) 1e6;
    for (long sTake = maxStake; sTake >= Math.max(maxStake - X, 0); sTake--) {
      ans = Math.max(ans, get(sTake));
    }
    for (int sTake = 0; sTake <= Math.min(maxStake, X); sTake++) {
      ans = Math.max(ans, get(sTake));
    }
    out.println(ans);
  }
